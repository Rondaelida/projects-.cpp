...
TEST(state_test, test_1) {
  ...
  EXPECT_EQ(model.state, s21::StatusSnake::StartGame);
  controller.userInput(Start, false);
  controller.updateCurrentState();
  EXPECT_EQ(model.state, s21::StatusSnake::Moving);
...
TEST(state_test, test_5) {
  ...
  controller.updateCurrentState();
  EXPECT_EQ(model.state, s21::StatusSnake::End);
}
...
TEST(state_test, test_7) {
  ...
  model.apple.y = 12;
  model.moveSnake();
  EXPECT_EQ(model.state, s21::StatusSnake::Spawn);
  controller.updateCurrentState();
  EXPECT_EQ(model.state, s21::StatusSnake::Moving);
}
...
TEST(state_test, test_8) {
  ...
  usleep(1000000);
  controller.updateCurrentState();
  EXPECT_EQ(model.state, s21::StatusSnake::Shifting);
  controller.updateCurrentState();
  EXPECT_EQ(model.state, s21::StatusSnake::Moving);
}
